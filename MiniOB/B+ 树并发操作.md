# B+ 树并发操作[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#b)
...
## B+ 树介绍[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#b_1)
...
## B+树的并发操作[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#b_2)

在多线程并发操作时，通常使用的手段是加锁，这里的实现方法也是这样。不过在学习并发B+树实现原理之前，需要对B+树的实现比较熟悉，有兴趣的同学可以网上搜索一下。

### Crabing Protocol[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#crabing-protocol)

在操作B+树时加对应的读写锁是一种最简单粗暴但是有效的方法，只是这样实现效率不高。于是就有一些研究创建了更高效的并发协议，并且会在协议设计上防止死锁的发生。 ![B+树示例](https://oceanbase.github.io/miniob/design/images/bplus-tree.jpg)
...
### MiniOB实现[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#miniob)

MiniOB的B+树并发实现方案与上个章节描述的方法是一致的。这里介绍一些实现细节。

> 在这里假设同学们对B+树的实现已经有了一定的了解。

#### B+树与Buffer Pool[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#bbuffer-pool)
...
那如何知道某个Frame关联的页面是否可以释放，然后可以与其它页面关联？ 如果这个Frame没有任何人使用，就可以重新关联到其它页面。这里使用的方法是引用计数，称为 `pin_count`。每次获取某个Frame时，`pin_count`就加1，操作完释放时，`pin_count`减1。如果`pin_count`是0，就可以将页面数据刷新到磁盘（如果需要的话），然后将Frame与磁盘文件的其它数据块关联起来。
...
#### 处理流程[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#_1)
...
#### 根节点处理[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#_2)

前面描述的几个操作，没有特殊考虑根节点。根节点与其它节点相比有一些特殊的地方： - B+树有一个单独的数据记录根节点的页面ID，如果根节点发生变更，这个数据也要随着变更。这个数据不是被Frame的锁保护的； - 根节点具有一定的特殊性，它是否“安全”，就是根节点是否需要变更，与普通节点的判断有些不同。

按照上面的描述，我们在更新（插入/删除）执行时，除了对节点加锁，还需要对记录根节点的数据加锁，并且使用独特的判断是否“安全的”方法。

在MiniOB中，可以参考`LatchMemo`，是直接使用xlatch/slatch对Mutex来记录加过的锁，这里可以直接把根节点数据保护锁，告诉LatchMemo，让它来负责相关处理工作。 判断根节点是否安全，可以参考`IndexNodeHandler::is_safe`中`is_root_node`相关的判断。

#### 如何测试[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#_3)
...
### 其它[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#_4)

#### 有条件的开启并发[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#_5)
...
#### 并发中的调试[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#_6)
...
## 参考[¶](https://oceanbase.github.io/miniob/design/miniob-bplus-tree-concurrency/#_7)
