...
RC PlainCommunicator::read_event(SessionEvent *&event)
{
  ...
  const int    max_packet_size = 1024 * 1024;  // 1MB支持大的TEXT字段
...
RC PlainCommunicator::write_tuple_result(SqlResult *sql_result)
{
  RC     rc    = RC::SUCCESS;
  Tuple *tuple = nullptr;
  LOG_DEBUG("write_tuple_result: starting to collect tuples");
  int tuple_count = 0;
  while (RC::SUCCESS == (rc = sql_result->next_tuple(tuple))) {
    assert(tuple != nullptr);
    tuple_count++;
    LOG_DEBUG("write_tuple_result: got tuple #%d: %s", tuple_count, tuple->to_string().c_str());

    int cell_num = tuple->cell_num();
    LOG_DEBUG("write_tuple_result: tuple has %d cells", cell_num);
    for (int i = 0; i < cell_num; i++) {
      if (i != 0) {
        const char *delim = " | ";
      ...
      string cell_str = value.to_string();
      LOG_DEBUG("write_tuple_result: cell[%d] = '%s'", i, cell_str.c_str());
  ...
  LOG_DEBUG("write_tuple_result: loop ended, collected %d tuples, final rc=%d", tuple_count, rc);
